...
ntfs_proc_idxentry(NTFS_INFO * a_ntfs, TSK_FS_DIR * a_fs_dir,
    ...
    uint32_t a_used_len, uint8_t process_deleted = 1)
{
    uintptr_t endaddr, endaddr_alloc, endaddr_stop;
    TSK_FS_NAME *fs_name;
	...
	/* check if only allocated space were requested. In this case processing
	 * of index allocated buffer will be terminated. This change is critical
	 * for performance when working with remote blob: memory traversal of 
	 * unallocated indexes leads to unnessesary downloads of remote pages */

	process_deleted ? endaddr_stop = endaddr : endaddr_stop = endaddr_alloc;
...
TSK_RETVAL_ENUM ntfs_process_dir_open_meta(TSK_FS_INFO * a_fs, TSK_FS_DIR ** a_fs_dir,
	TSK_INUM_T a_addr, uint8_t process_deleted = 1)
{
...
ntfs_dir_open_meta(TSK_FS_INFO * a_fs, TSK_FS_DIR ** a_fs_dir,
    TSK_INUM_T a_addr)
{
	return ntfs_process_dir_open_meta(a_fs, a_fs_dir, a_addr);
}

TSK_RETVAL_ENUM ntfs_dir_open_meta_remote(TSK_FS_INFO * a_fs, TSK_FS_DIR ** a_fs_dir,
	TSK_INUM_T a_addr)
{
	return ntfs_process_dir_open_meta(a_fs, a_fs_dir, a_addr, 0);
}
